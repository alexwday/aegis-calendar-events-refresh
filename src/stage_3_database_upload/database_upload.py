...
OUTPUT_CSV = OUTPUT_DIR / "calendar_events.csv"
OUTPUT_SQL = OUTPUT_DIR / "calendar_events.sql"
...
def export_sql_dump(events, output_path):
    """Export events as SQL INSERT statements."""
    if not events:
        log.warning("No events to export to SQL")
        return False
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        # Header comment
        f.write(f"-- Calendar Events SQL Dump\n")
        f.write(f"-- Generated by Stage 3: Database Upload\n")
        f.write(f"-- Table: {table}\n")
        f.write(f"-- Records: {len(events)}\n\n")
        ...
        # Insert statements
        f.write(f"-- Insert {len(events)} records\n")
        col_names = ", ".join(columns)

        for event in events:
            values = ", ".join(
                escape_sql_value(event.get(col, "")) for col in columns
            )
            f.write(f"INSERT INTO {table} ({col_names}) VALUES ({values});\n")

        f.write(f"\n-- End of dump\n")

    log.info("Exported SQL dump: %s", output_path)
    return True
...
def export_csv(events, output_path):
    ...
    if not events:
        log.warning("No events to export to CSV")
        return False
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for event in events:
            row = {col: event.get(col, "") for col in EXPECTED_COLUMNS}
            writer.writerow(row)

    log.info("Exported CSV: %s", output_path)
    return True
...
def main():
    ...
    # Export data files (always, even in dry-run mode)
    log.info("Exporting data files...")
    export_csv(events, OUTPUT_CSV)
    export_sql_dump(events, OUTPUT_SQL)
    log.info("Output files saved to: %s", OUTPUT_DIR)
